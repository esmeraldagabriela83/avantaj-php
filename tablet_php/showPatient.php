...
<div class="container">
    ...
    <label for="patientName">Patient name</label><br>
                <input type="text" name="patientNameUserWritten"  class="form-control"  id="patientName" value="<?php
                if( isset($_POST['patientNameUserWritten']) ){
                echo $_POST['patientNameUserWritten'] ;
                }
                ?>"/><br/>
...
if(isset($_POST['showPatientWithSelectedAge'])){
    ...
    echo '<h3>You set patient name</h3>' ;

    echo '<h3>Your patient is : ' . $_POST['patientNameUserWritten'] . '</h3>';

    $patientNameUserWritten=$_POST['patientNameUserWritten'];
   

          //log in cu select
        // daca da - realizam un query SELECT pe baza de date
        $selectie_db = mysqli_query($c_db, "SELECT * FROM patienttable WHERE name = $patientNameUserWritten");
